...
import net.kccricket.bestesttool.tags.v1_17;
    ...
    void initMap() {
        ...
        for (Material mat : Material.values()) {
            ...
            if(matName.contains("GLASS")) {
                addToMap(matName, Tool.NONE);
            }
            if(matName.contains("CARPET")) {
                addToMap(matName, Tool.NONE);
            }
            if (matName.contains("AMETHYST")) { // 1.17
                addToMap(matName, Tool.PICKAXE);
            }
            if(matName.endsWith("_ORE")) { // 1.17
                addToMap(matName, Tool.PICKAXE);
            }
            if(matName.contains("BASALT")) { // 1.17
                addToMap(matName, Tool.PICKAXE);
            }
            if(matName.contains("DEEPSLATE")) { // 1.17
                addToMap(matName, Tool.PICKAXE);
            }
        }
